...
_start:
    ...
    | ── Main sweep loop: 128 swap pairs ──────────────────────────────
    | d5 = iteration index i (0..127)
    moveq   #0, %d5
_iter_loop:
    | Dirty (i mod 128) dcache lines in RAM (copyback pages).
    move.l  %d5, %d6
    andi.l  #0x7F, %d6
    beq     .no_dirty
    lea     0x00104000, %a1
...
.no_dirty:
    ...
    | Per-iteration alignment delay: i extra loop turns (~1 cycle
    | granularity phase sweep of the swap vs any background drain).
    move.l  %d5, %d0
.delay_loop:
    subq.l  #1, %d0
    bpl     .delay_loop
