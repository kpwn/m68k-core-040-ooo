| tmp1_reuse_loop_then_crack_pressure.s -- sibling of
| tmp1_reuse_loop_then_crack.s that adds physical-register pressure:
| D0-D5/A0/A1 are kept live with distinct values across every outer
| iteration, and the inner TMP1-reuse loop interleaves independent
| ALU busywork on D1-D5 (rotated each iteration) alongside the
| move.b (%a2),(%a2) TMP1 chain, so the ROB/rename window around the
| crack has more concurrently in-flight, physical-register-hungry work
| competing for the shared free-list -- checking whether reduced free-
| phys-reg headroom (as opposed to pure adjacency/timing) is a
| necessary ingredient of the hypothesised race.
|
...
_start:
    ...
    | Physical-register pressure: permanently map D2-D5/A0/A1 to live,
    | distinct values BEFORE the outer loop starts.  These stay
    | architecturally mapped (RAT entries persist) for the rest of the
    | run, permanently shrinking the free physical-register pool the
    | TMP1-reuse loop and the crack draw from.
...
_iter_loop:
    ...
    | for the real ROM's 408700f8/408700fc dbmi-loop shape.  Interleaved
    | with independent ALU busywork on D2-D5 (its own true-dependency
    | chain, touches neither A2/D6 loop-control nor REG_TMP1) so more
    | concurrently in-flight, physical-register-hungry work competes
    | with the TMP1 chain for rename/issue bandwidth every iteration.
    lea     SCRATCH_ADDR, %a2
...
_tmp1_loop:
    ...
    add.l   %d2, %d3              | independent busywork, own dep chain
    eor.l   %d4, %d5              | independent busywork, own dep chain
    dbf     %d6, _tmp1_loop
