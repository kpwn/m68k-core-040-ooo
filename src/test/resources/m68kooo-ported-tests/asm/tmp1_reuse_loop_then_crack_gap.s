| tmp1_reuse_loop_then_crack_gap.s -- sibling of
| tmp1_reuse_loop_then_crack.s that more faithfully reproduces the REAL
| ROM control-flow gap between the TMP1-reuse loop and the crack.
| Careful disassembly of the actual path (files/420dbff3.rom,
| 0x408700f8-0x40870132) shows the "successful" exit from the
| 408700f8/408700fc dbmi loop does NOT fall straight into the crack --
| it passes through several more instructions first, including a real
| BSR to a subroutine at 0x4086fdd8 and a SECOND poll loop (`cmpib
| #-86,%a3@` against a fixed byte) before finally reaching the `bras
| 0x408701a2` that lands on the crack's epilogue.  This variant
| recreates that SHAPE (loop -> call a leaf subroutine -> a short
| poll loop -> land on the crack) without depending on real IWM timing,
| to check whether the intervening BSR/poll activity (as opposed to
| pure zero-gap adjacency, covered by the base/_bubble variants) is
| itself a necessary ingredient of the hypothesised race.
|
...
_tmp1_loop:
    ...
    | ROM-faithful gap: BSR to a leaf subroutine (no REG_TMP1 use, plain
    | ALU-only), then a short poll loop comparing a memory byte -- shape
    | match for the real 0x40870112 bsrw / 0x40870122-0x4087012a poll
    | loop found in the disassembly between this loop and the crack.
    bsr     _dummy_helper
    move.w  #4, %d7
_poll_loop:
    cmp.b   #0x5A, SCRATCH_ADDR
    beq     _poll_done
    subq.w  #1, %d7
    bne     _poll_loop
_poll_done:

    | THE CRACK UNDER TEST, after the ROM-faithful gap above.
...
| Leaf subroutine standing in for the real ROM's 0x4086fdd8 callee --
| plain ALU-only, deliberately avoids REG_TMP1 (no memory-to-memory
| moves, no postinc/predec EAs) so it contributes ONLY pipeline-depth /
| dispatch-window pressure, not another TMP1 writer.
_dummy_helper:
    move.l  %d5, -(%sp)
    moveq   #1, %d5
    add.l   %d5, %d5
