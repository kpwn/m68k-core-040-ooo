| tmp1_reuse_loop_then_crack_bubble.s -- sibling of
| tmp1_reuse_loop_then_crack.s with a few NOP bubbles inserted between
| the TMP1-reuse loop's last iteration and the crack under test, to
| check whether the hypothesised RAT/allocator race needs true
| zero-gap adjacency or still reproduces with a couple cycles of slack
| (the real ROM's actual entry path to the crack, per disassembly, is
| NOT perfectly zero-gap either -- see tmp1_reuse_loop_then_crack_gap.s
| for the more ROM-faithful longer-gap variant).
|
...
_tmp1_loop:
    ...
    | Bubble variant: a few cycles of slack (NOPs touch no registers the
    | test cares about) between the TMP1-reuse loop's last iteration and
    | the crack under test, instead of true zero-gap adjacency.
    nop
    nop
    nop

    | THE CRACK UNDER TEST, after the bubble above.
