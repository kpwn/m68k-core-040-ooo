...
/** Task 1: committed architectural system state — SR system byte (S/I/T), VBR,
  ...
  *  - usp / isp: the two A7 banks tested here (M=0; reset: only isp is the boot SP).
  *  - a7Out: the architectural A7 value, muxed by committed S (S ? isp : usp).
  *
  * Write ports (driven by the exception FSM / RTE / privileged moves at commit):
  *  - setSrSys/setVbr/setUsp/setIsp: simple committed-register writes.
  *  - writeA7: writes the BANK selected by committed S (so a handler's pushes to
  *    A7 update ISP while in supervisor; user pushes update USP).
  */
class SystemStateSpec extends AnyFunSuite {
  ...
  class Dut extends Component {
    ...
    val setIspV   = in Bool (); val setIspD   = in UInt (32 bits)
    ...
    ss.setIsp.valid   := setIspV;   ss.setIsp.payload   := setIspD
  ...
  def idle(dut: Dut): Unit = {
    ...
    dut.setIspV #= false; dut.setIspD #= 0
  ...
  test("reset state: srSys=0x27, vbr=0, A7 selects SSP (S=1)") {
    M68kSim().compile(new Dut).doSim { dut =>
      ...
      dut.setIspV #= true; dut.setIspD #= 0x00100000L
  ...
  test("writeA7 updates the bank selected by committed S") {
    M68kSim().compile(new Dut).doSim { dut =>
      ...
      idle(dut)
      dut.clockDomain.waitSampling()
      assert(dut.ispOut.toLong == 0x00200000L, f"isp=0x${dut.ispOut.toLong}%x")
      ...
      assert(dut.ispOut.toLong == 0x00200000L)
