...
trait CoreBenchHarness extends AnyFunSuite {
  ...
  /** The three physical memories behind the core, handed to `Kernel.prepMem` so a
    ...
    *  - `dWalkMem`  : behind `dtlb.walkerAxi` (D-side page tables).
    *  - `iWalkMem`  : behind `itlb.walkerAxi` (I-side page tables).
    * On current HEAD the walkers have their OWN AXI ports and do NOT go through
    * the D-cache, so `dmem` and `dWalkMem` are genuinely separate memories. */
