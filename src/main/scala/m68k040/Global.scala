...
object Global {
  ...
  /** THE int physical-register pool size — the ONE number, in plain-constant form.
    ...
    * host.
    *
    * MERGE NOTE (2026-09-17, master <- integrate/all-fixes). The two sides of this merge
    * disagreed on the VALUE, not the mechanism: the 200 MHz closure line single-sourced
    * the pool at 50, `master` restated 54 in three independent places (`Config.physInt`,
    * `RenameStage.intFree.physCount`, `RegfileSpec.Int.depth`) with no guard. Both were
    * self-consistent on their own branch, so the `require` below fires on a naive merge --
    * which is exactly its job. Reconciled deliberately at **54**, the owner's chosen pool
    * size and the value `master` already carried everywhere; the closure line's
    * single-source-plus-`require` MECHANISM is what was kept. `log2Up(54) == log2Up(50) == 6`,
    * so every derived id width (`M68kParams.physIntIdWidth`, `pdst`, `psrc*`) is unchanged
    * by this reconciliation -- only the freelist's initial pool and the PRF `Mem` depth
    * grow, which is the direction that is SAFE (a pool smaller than the bitmaps/PRF wastes
    * ids; a pool larger than them is the silent wrong-operand bug described above). */
  val PHYS_INT_REGS_DEFAULT: Int = 54
