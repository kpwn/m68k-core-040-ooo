...
/** Dispatch-owned metadata only; store bytes remain owned by the LSU/SQ.
...
class MemoryDependencyTracker(entries: Int = 8) extends Component {
  ...
  val io = new Bundle {
    ...
    val flush = in Bool()
    val robHead = in UInt(m68k040.Global.ROB_ID_W_DEFAULT bits)
  ...
  val robIds = Vec.fill(entries)(Reg(UInt(m68k040.Global.ROB_ID_W_DEFAULT bits)) init 0)
  ...
  for (n <- 0 until entries) {
    ...
    for (lane <- 0 until 2) {
      when(io.reserve.fire && selected(lane)(n) && (if(lane == 0) True else io.reserveSecond)) {
        ...
        robIds(n) := io.reserve.payload(lane).robId
  ...
  val queryAge = robIds(q) - io.robHead
  for (n <- 0 until entries) {
    val age = robIds(n) - io.robHead
    ...
    r.older := occupied(n) && !canceled(n) && q =/= n &&
      (committed(n) || irreversible(n) || age < queryAge)
