...
set names [dict create n0 {data[0]} n1 {data[1]} n2 reset n3 tie n4 external n5 multi]
...
set pip_map [dict create n0 {p0 p1 p2} n1 {p3 p4} n2 {p5 p6} n3 {} n4 {p7} n5 {p8}]
set pins [dict create n0 {q0 i0 i1} n1 {q1 i2} n2 {q2 i3 i4 i5} n3 {q3 i6} n4 {i7} n5 {q0 q1 i8}]
proc open_checkpoint {args} {}
proc get_nets {args} {
    # Canonical query deliberately repeats a physical net to test deduplication.
    if {[lsearch -exact $args -segments] >= 0} {return {n0 n0 n1 n2 n3 n4 n5}}
...
check {[string match {*canonical_nets=6*} $summary]} "hierarchy alias double counted"
check {[string match {*pip_assignments=9*} $summary]} "PIP reconciliation"
