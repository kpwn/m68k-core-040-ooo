...
**SUPERSEDED by this task's own later fix (`55b97bd`) — no longer accurate, kept for history only.** The premise above ("no existing test can distinguish T from T+1", "no mutation run: there is no test to fail") was corrected during Task 12's review: `IcachePrefetchSpec`'s "P1 residual (cycle T)" test drives `ICacheModeTranslationPlugin` to a live INHIBITED/FAULT transition on an in-range window and asserts on cycle T directly (safety bound) as well as T+1 (non-vacuity) — see `IcachePlugin.scala`'s own residual comment and `IcacheMutationProofSpec`'s sixth, non-vacuous entry for the corrected, current record. The action item below (an "optional" directed test) was in fact completed and made non-optional in the shipped test suite.

**Required action here (as originally written, historical):** no mutation run (there is no test to fail). Copy the scope statement above into `IcacheMutationProofSpec`'s scaladoc as a sixth, explicitly **UNCOVERED** entry, so the gap is recorded as a known-and-bounded property rather than an unexamined one. Optionally add a directed test using `ICacheModeTranslationPlugin` (already used by `IcacheUnifiedArraySpec`) driving an INHIBITED transition on a live window; if added, it should pin the T+1 closure (which is testable) rather than attempt to assert on T.
